...
if start_clicked:
    ...
    cfg = GSRConfig()  # baud=9600, smooth_window=50, etc.
...
# Refresh while streaming (1 Hz is demo-stable)
if st.session_state.gsr_stream is not None and st.session_state.gsr_stream.is_running():
    st_autorefresh(interval=200, key="gsr_autorefresh")


# ------------------------------------------------------------
# Main UI
# ------------------------------------------------------------
st.title("IMAGS-lite: Audio + GSR Feature Viewer")
st.caption("Analyze a song (tempo + loudness/energy trends) and view live GSR concurrently. No playback yet.")


# ------------------------------------------------------------
# Live GSR plot (always shown)
# ------------------------------------------------------------
...
if st.session_state.gsr_stream is None:
    ...
    if not pts:
        ...
        if len(gsr_df) > 2:
            t_now = float(gsr_df["t_sec"].iloc[-1])
...
# ------------------------------------------------------------
# Audio analysis controls (DO NOT st.stop() the whole app)
# ------------------------------------------------------------
...
with c3:
    clear_audio = st.button("Clear audio results")
...
if analyze_btn and uploaded is not None:
    ...
    # Persist results so they survive reruns (NEW)
    st.session_state.audio_res = res
    st.session_state.audio_df = df


# ------------------------------------------------------------
# Render audio results if available (survives reruns)
# ------------------------------------------------------------
if st.session_state.audio_res is None:
    st.info("Upload a file and click **Analyze song** to compute features.")
else:
